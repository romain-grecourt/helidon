...
public class IdcsRoleMapperProvider extends IdcsRoleMapperProviderBase implements SubjectMappingProvider {
    ...
    protected IdcsRoleMapperProvider(Builder<?> builder) {
        ...
        this.appToken = new AppToken(oidcConfig.appClient(), tokenEndpointUri, oidcConfig.tokenRefreshSkew());
    }
    ...
    protected Subject enhance(ProviderRequest request, AuthenticationResponse previous, Subject subject) {
        ...
        List<Grant> newGrants = roleCache.computeValue(username, () -> Optional.of(List.copyOf(computedGrants)))
                                         .orElseGet(List::of);
        List<? extends Grant> additionalGrants = addAdditionalGrants(subject, newGrants);
        newGrants.addAll(additionalGrants);
    ...
    protected List<? extends Grant> getGrantsFromServer(Subject subject) {
        ...
        return appToken.getToken(tracing)
                .map(appToken -> {
                 ...
                 jsonBuilder.add("schemas", arrayBuilder);
                 ...
                 try {
                     return Contexts.runInContext(childContext, () -> {
                         Http1ClientRequest post = oidcConfig().generalClient()
                                                               .post()
                                                               .uri(asserterUri)
                                                               .headers(it -> {
                         ...
                         return processRoleRequest(post, jsonBuilder.build(), subjectName);
                     });
                 } catch (Throwable ex) {
                     tracing.error(ex.getMessage());
                     throw ex;
                 } finally {
                     tracing.finish();
                 }
