...
public class GreetService implements HttpService {
    ...
    private void connectionClose(ServerRequest request, ServerResponse response) throws InterruptedException {
        OutputStreamWriter writer = new OutputStreamWriter(response.outputStream());
        CountDownLatch latch = new CountDownLatch(2);
        try (ExecutorService execSvc = Executors.newSingleThreadScheduledExecutor()) {
            execSvc.execute(() -> {
                for (int i = 0; i < 15; i++) {
                    try {
                        Thread.sleep(500);
