...
public class SimpleInsertIT extends AbstractIT {
    ...
    public void testNamedInsertOrderArgs() {
        ...
        try (DbExecute exec = DB_CLIENT.execute()) {
            long result = exec.namedInsert("insert-pokemon-order-arg", pokemon.getId(), pokemon.getName());
            verifyInsertPokemon(result, pokemon);
        }
    }
    ...
    public void testInsertOrderArgs() {
        ...
        try (DbExecute exec = DB_CLIENT.execute()) {
            long result = exec.insert(INSERT_POKEMON_ORDER_ARG, pokemon.getId(), pokemon.getName());
            verifyInsertPokemon(result, pokemon);
        }
    }
