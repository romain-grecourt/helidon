...
class SchemaGenerator {
    ...
    Annotation type(PrototypeInfo prototypeInfo, List<OptionHandler> handlers) {
        ...
        if (!key.isBlank()) {
            if (configured.root()) {
                builder.putProperty("root", AnnotationProperty.create(true));
            }
            builder.putProperty("prefix", AnnotationProperty.create(key));
        }

        var description = typeDescription(prototypeInfo);
        builder.putProperty("description", AnnotationProperty.create(description));

        var provides = providedTypes(prototypeInfo);
        if (!provides.isEmpty()) {
            builder.putProperty("provides", AnnotationProperty.create(provides));
        }

        var options = options(prototypeInfo, handlers);
        if (!options.isEmpty()) {
            builder.putProperty("options", AnnotationProperty.create(options));
        }
    ...
    private Annotation option(PrototypeInfo prototypeInfo, OptionInfo optionInfo) {
        ...
        if (configured.merge()) {
            if (typeName.equals(TypeNames.STRING) || typeName.unboxed().primitive()) {
                ...
                builder.putProperty("type", AnnotationProperty.create(typeName));
                builder.putProperty("mergeWithParent", AnnotationProperty.create(true));
                return builder.build();
            }
        }

        builder.putProperty("key", AnnotationProperty.create(key));
        builder.putProperty("description", AnnotationProperty.create(description));
        builder.putProperty("type", AnnotationProperty.create(typeName));
        ...
        if (!"VALUE".equals(kind)) {
            ...
            builder.putProperty("kind", AnnotationProperty.create(enumValue));
        }

        var defaultValue = defaultValue(optionInfo);
        if (defaultValue != null) {
            builder.putProperty("value", AnnotationProperty.create(defaultValue));
        }

        if (optionInfo.required()) {
            builder.putProperty("required", AnnotationProperty.create(true));
        }

        if (optionInfo.provider().isPresent()) {
            builder.putProperty("provider", AnnotationProperty.create(true));
        }

        if (optionInfo.deprecation().isPresent()) {
            builder.putProperty("deprecated", AnnotationProperty.create(true));
        }

        var allowedValues = allowedValues(typeInfo, optionInfo);
        if (!allowedValues.isEmpty()) {
            builder.putProperty("allowedValues", AnnotationProperty.create(allowedValues));
        }
        return builder.build();
    }
    ...
    private List<Annotation> allowedValues(List<OptionAllowedValue> allowedValues) {
        ...
        for (var e : allowedValues) {
            result.add(Annotation.builder()
                    ...
                    .putProperty("value", AnnotationProperty.create(e.value()))
                    .putProperty("description", AnnotationProperty.create(e.description()))
                    .build());
        }
        return result;
    }
        ...
        for (var e : typeInfo.elementInfo()) {
            if (e.kind() == ElementKind.ENUM_CONSTANT) {
                ...
                result.add(Annotation.builder()
                        ...
                        .putProperty("description", AnnotationProperty.create(description))
                        .build());
            }
        }
        return result;
