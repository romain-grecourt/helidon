...
public class TestCORS {
    ...
    void testGreetingChangeWithCors() {
        ...
        Http1ClientResponse res = req.path("/greet/greeting")
                                     .request();

        assertThat("pre-flight status", res.status().code(), is(200));
        Headers preflightResponseHeaders = res.headers();
        ...
        res = putResponse("/greet/greeting", "Cheers", req);
    ...
    void testGreetingChangeWithCorsAndOtherOrigin() {
        ...
        Http1ClientResponse r = putResponse("/greet/greeting", "Ahoy", req);
