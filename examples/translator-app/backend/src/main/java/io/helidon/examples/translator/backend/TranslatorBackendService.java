...
public class TranslatorBackendService implements HttpService {
    ...
    private void getText(ServerRequest req, ServerResponse res) {
        ...
        String translation;
        switch (language) {
            case CZECH -> translation = TRANSLATED_WORDS_REPOSITORY.get(query + SEPARATOR + CZECH);
            case SPANISH -> translation = TRANSLATED_WORDS_REPOSITORY.get(query + SEPARATOR + SPANISH);
            case CHINESE -> translation = TRANSLATED_WORDS_REPOSITORY.get(query + SEPARATOR + CHINESE);
            case HINDI -> translation = TRANSLATED_WORDS_REPOSITORY.get(query + SEPARATOR + HINDI);
            case ITALIAN -> translation = TRANSLATED_WORDS_REPOSITORY.get(query + SEPARATOR + ITALIAN);
            case FRENCH -> translation = TRANSLATED_WORDS_REPOSITORY.get(query + SEPARATOR + FRENCH);
            default -> {
                res.status(404)
                   .send(String.format(
