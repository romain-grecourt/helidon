...
public final class Main {
    ...
    public static void main(String[] args) {
        ...
        WebServer.Builder builder1 = WebServer.builder();
        setupConfigBased(builder1, config);
        WebServer server1 = builder1.start();
        System.out.println("Started config based WebServer on http://localhost:" + server1.port());

        WebServer.Builder builder2 = WebServer.builder();
        setupBuilderBased(builder2, config);
        WebServer server2 = builder2.start();
        System.out.println("Started builder based WebServer on http://localhost:" + server2.port());
    }

    static void setupBuilderBased(WebServer.Builder server, Config config) {
        server.config(config)
              .routing(Main::routing)
              .tls(tls -> tls.privateKey(keystore ->
                      keystore.keystore(Resource.create("certificate.p12"))
                              .keystorePassphrase("helidon")));
    }

    static void setupConfigBased(WebServer.Builder server, Config config) {
        server.config(config)
              .routing(Main::routing);
    }
