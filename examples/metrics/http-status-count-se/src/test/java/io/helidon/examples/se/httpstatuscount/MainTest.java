...
public class MainTest {
    ...
    @SetUpServer
    public static void setup(WebServer.Builder server) {
        Main.setup(server);
    }

    @Test
    public void testMicroprofileMetrics() {
        try (Http1ClientResponse response = client.get("/simple-greet/greet-count")
                                                  .request()) {
            assertThat(response.as(String.class), containsString("Hello World!"));
        }

        try (Http1ClientResponse response = client.get("/metrics")
                                                  .request()) {
    ...
    public void testHealth() {
        try (Http1ClientResponse response = client.get("health")
                                                  .request()) {
            assertThat(response.status().code(), is(200));
        }
    }
    ...
    public void testSimpleGreet() {
        try (Http1ClientResponse response = client.get("/simple-greet")
                                                  .request()) {
