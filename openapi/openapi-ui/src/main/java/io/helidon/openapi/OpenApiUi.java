...
final class OpenApiUi implements OpenApiService, RuntimeType.Api<OpenApiUiConfig> {
    ...
    OpenApiUi(OpenApiUiConfig config) {
        ...
        options.put(Option.url, config.webContext());  // location of the OpenAPI document
    ...
    @Override
    public void setup(HttpRules rules, Function<MediaType, String> content) {
        if (!config.isEnabled()) {
            return;
        }
        String webContext = config.webContext();
        rules.get(webContext + "/", (req, res) -> handle(req, res, content))
                .get(webContext + "/index.html", this::index)
                .register(webContext, StaticContentService.create("helidon-openapi-ui"))
                .register(webContext, StaticContentService.create("META-INF/resources/openapi-ui"));
    }
        ...
        req.headers()
                ...
                .ifPresentOrElse(ct -> {
                    if (TEXT_HTML.test(ct)) {
                        ...
                        res.header(Http.HeaderNames.LOCATION, config.webContext() + "/index.html");
                        res.send();
                    } else {
                        res.headers().contentType(ct);
