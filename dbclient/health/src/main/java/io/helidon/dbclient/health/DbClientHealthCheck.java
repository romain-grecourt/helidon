...
public abstract class DbClientHealthCheck implements HealthCheck {
    ...
    static final class DbClientHealthCheckAsNamedQuery extends DbClientHealthCheck {
        ...
        protected void execPing() {
            try (DbExecute exec = dbClient().execute()) {
                exec.namedQuery(statementName).forEach(it -> {});
            }
        }
    ...
    static final class DbClientHealthCheckAsQuery extends DbClientHealthCheck {
        ...
        protected void execPing() {
            try (DbExecute exec = dbClient().execute()) {
                exec.query(statement).forEach(it -> {});
            }
        }
    ...
    public static final class Builder implements io.helidon.common.Builder<Builder, DbClientHealthCheck> {
         ...
         * Default uses {@link DbClient#dbType()}.
         *
        ...
        public Builder config(final Config config) {
            ...
            config.get(CONFIG_TYPE)
                    ...
                    .ifPresent(type -> {
                        ...
                        switch (stmtType) {
                            case DML -> this.isDML = true;
                            case QUERY -> this.isDML = false;

                            // Code consistency check to make sure all HealthCheckStMtType values are handled.
                            default -> throw new HealthCheckBuilderException(
                                    String.format("Unknown HealthCheckStMtType instance: %s",
                                            stmtType.typeName()));
                        }
                    });
            try {
