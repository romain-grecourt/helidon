 ...
 * the {@link DbClientProviderBuilder} from Helidon DB API.
 */
public final class JdbcDbClientProviderBuilder implements DbClientProviderBuilder<JdbcDbClientProviderBuilder> {
    ...
    public DbClient build() {
        if (null == connectionPool) {
            if (null == url) {
                throw new DbClientException("No database connection configuration (%s) was found."
                        + " Use \"connection\" configuration key,"
                        + " or configure on builder using \"connectionPool(ConnectionPool)\"");
            }
