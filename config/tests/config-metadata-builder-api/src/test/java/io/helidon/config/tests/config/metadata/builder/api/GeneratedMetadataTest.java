...
class GeneratedMetadataTest {
    ...
    void testMetadata() {
        ...
        var actual = Json.createReader(is).readArray();
    ...
    static Matcher<JsonValue> hasBoolean(String name, Matcher<Boolean> matcher) {
        return new FeatureMatcher<>(matcher, "has property " + name, name) {
            @Override
            protected Boolean featureValueOf(JsonValue target) {
                return target.asJsonObject().getBoolean(name);
            }
        };
    }
}
