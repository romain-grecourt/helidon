...
class TypeHandler {
    ...
    void handle(TypeInfo annotatedTypeInfo) {
        ...
        if (model == null) {
            var annot = annotatedTypeInfo.annotation(TypeNames.CONFIGURED);
            boolean ignoreBuilderMethod = annot.booleanValue("ignoreBuildMethod").orElse(false);
            var targetTypeInfo = ignoreBuilderMethod ? annotatedTypeInfo : targetType(annotatedTypeInfo);
        ...
        for (var e : typeInfo.elementInfo()) {
            if (isMethod(e)) {
                ...
                if (typeName.equals(enclosingTypeName)) {
                    if (e.hasAnnotation(TypeNames.OPTIONS)) {
                        var annots = e.annotation(TypeNames.OPTIONS).annotationValues().orElseGet(List::of);
                        for (var a : annots) {
                            if (a.booleanValue("configured").orElse(true)) {
                                options.add(option(a, typeInfo, e));
                            }
                        }
                    } else if (e.hasAnnotation(TypeNames.OPTION)) {
                        var a = e.annotation(TypeNames.OPTION);
                        if (a.booleanValue("configured").orElse(true)) {
                            options.add(option(e.annotation(TypeNames.OPTION), typeInfo, e));
                        }
                    }
                }
            }
    ...
    private TypeName optionTypeName(TypeData data) {
        ...
        var rawTypeName = typeName.genericTypeName();
        if (typeName.isList() || typeName.isSet() || rawTypeName.equals(TypeNames.ITERABLE)) {
        ...
        } else if (typeName.equals(TypeNames.CHAR_ARRAY)) {
            ...
            var iterableTypeInfo = resolver.superType(typeInfo, TypeNames.ITERABLE);
            ...
            // warn map subtypes
            var mapTypeInfo = resolver.superType(typeInfo, TypeNames.MAP);
            if (mapTypeInfo != null) {
    ...
    private TypeData optionTypeData(Annotation annot, Object originElement) {
        ...
        if (typeName.equals(TypeNames.OPTION)) {
    ...
    private TypeName mapOptionTypeName(TypeData optionType) {
        ...
        if (typeArgs.size() == 2) {
            ...
            if (!keyTypeName.equals(TypeNames.STRING)) {
    ...
    private CmOption.Kind optionKind(TypedElementInfo methodInfo) {
        ...
        if (typeName.array() || typeName.isList() || typeName.isSet() || typeName.equals(TypeNames.ITERABLE)) {
    ...
    private Set<TypeInfo> configuredAncestors(TypeInfo typeInfo) {
        return configuredAncestors.computeIfAbsent(typeInfo, k ->
                resolver.typeHierarchy(k, it -> it != k && it.hasAnnotation(TypeNames.CONFIGURED)));
    }
    ...
    private static TypeData methodTypeData(TypedElementInfo methodInfo) {
        ...
        if (args.size() == 1) {
            ...
            if (typeName.equals(TypeNames.BOXED_VOID)) {
    ...
    private static Object parseValue(TypeName typeName, String defaultValue) {
        if (typeName.equals(TypeNames.BOXED_DOUBLE)) {
