...
public final class JavadocWriter {
    ...
    public void write(JavadocTree element) {
        switch (element) {
            case EltStart(var t, var c, var a) -> writeStartElement(t, c, a);
            case EltClose(var t) -> writeEndElement(t);
            case Text(var v) -> buf.append(encode(buf.isEmpty() ? v.stripLeading() : v));
            case Escape(var v) -> buf.append(encode(v));
            case InlineTag(var t, var b) -> {
                switch (t) {
                    ...
                    case "literal" -> buf.append(encode(b));
                    case "summary" -> {
