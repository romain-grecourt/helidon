...
public final class JavadocReader {
        ...
        while (parser.hasNext()) {
            ...
            switch (event) {
                case Event.EltStart(var s) -> {
                    ...
                    var e = new EltStart(s, selfClosing, attrs);
                ...
                case Event.InlineTag(var s) -> {
                    var e = new InlineTag(s, readInlineBody());
                    switch (s) {
                        case "return", "summary" -> {
                            elements.add(e);
                            if (elements == fs) {
                ...
                case Event.BlockTag(var s) -> blockTags.add(new BlockTag(s, readBlockBody()));
                case Text n -> {
                    if (elements == fs) {
                        var s = n.value();
                        int sbreak  = sentenceBreak(s);
                        if (sbreak > 0) {
                            fs.add(new Text(s.substring(0, sbreak).stripTrailing()));
                            int offset = skipWhiteSpace(s, sbreak);
                            if (offset > 0) {
                                body.add(new Text(s.substring(offset)));
                            }
                            elements = body;
                            break;
                        }
    ...
    private List<JavadocTree> readBlockBody() {
        ...
        while (parser.hasNext()) {
            ...
            parser.skip();
            var n = switch (event) {
                case Event.InlineTag(var s) -> new InlineTag(s, readInlineBody());
    ...
    private String readInlineBody() {
        ...
        while (parser.hasNext()) {
            var event = parser.peek();
            if (event instanceof Text(var s)) {
                body = s;
                parser.skip();
            } else {
                return body;
