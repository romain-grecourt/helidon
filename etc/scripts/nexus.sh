...
nexus_start() {
  ...
  curl -s \
    ...
    -w "%{stderr}%{http_code} %{url}\n%output{${statusfile}}%{http_code}" \
    -H 'Content-Type: application/json' \
    -d "{\"data\": {\"description\": \"${1}\"}}" \
    -o "${tmpfile}" \
    "${NEXUS_URL}/service/local/staging/profiles/${STAGING_PROFILE}/start"
...
nexus_finish() {
  ...
  curl -s \
    ...
    -o "${tmpfile}" \
    "${NEXUS_URL}/service/local/staging/profiles/${STAGING_PROFILE}/finish"
  ...
  while true ; do
    ...
    curl -s \
      ...
      -o "${tmpfile}" \
      "${NEXUS_URL}/service/local/staging/repository/${1}"
...
nexus_upload() {
  ...
  curl -s \
    ...
    --write-out "%{stderr}%{http_code} %{url}\n" \
    --config "${tmpfile}" \
    --parallel \
    --retry 3
}
